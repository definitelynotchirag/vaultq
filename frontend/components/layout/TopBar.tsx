...
export function TopBar({ onSearch, searchQuery }: TopBarProps) {
  ...
  return (
    <div className="h-16 bg-white border-b border-[#e5e5e5] shadow-[0_1px_2px_0_rgba(0,0,0,0.05)] flex items-center justify-between px-3 md:px-6 fixed top-0 left-0 right-0 z-50">
      <div className="flex items-center gap-3 md:gap-6 flex-1">
        <div className="flex items-center gap-2 md:gap-3 w-[140px] md:w-[200px] shrink-0">
          ...
          <span className="text-lg md:text-[22px] text-[#5f6368] font-normal" style={{ fontFamily: 'var(--font-sans)' }}>
            VaultQ
          </span>
        </div>
      ...
      <div className="flex items-center gap-1 md:gap-2 shrink-0">
        <button className="w-8 h-8 md:w-10 md:h-10 rounded-full hover:bg-gray-100 flex items-center justify-center transition-colors">
          <HelpCircle className="w-4 h-4 md:w-5 md:h-5 text-[#5f6368]" />
        </button>
        <button className="w-8 h-8 md:w-10 md:h-10 rounded-full hover:bg-gray-100 flex items-center justify-center transition-colors hidden sm:flex">
          <Settings className="w-4 h-4 md:w-5 md:h-5 text-[#5f6368]" />
        </button>
        <button className="w-8 h-8 md:w-10 md:h-10 rounded-full hover:bg-gray-100 flex items-center justify-center transition-colors hidden sm:flex">
          <Grid3x3 className="w-4 h-4 md:w-5 md:h-5 text-[#5f6368]" />
        </button>
        <div className="relative ml-1 md:ml-2" ref={userMenuRef}>
          ...
          {showUserMenu && (
            <div className="absolute right-0 top-11 w-64 bg-white border border-[#e5e5e5] rounded-lg shadow-lg py-2 z-50">
              <div className="px-4 py-3 border-b border-[#e5e5e5]">
                <div className="text-[#202124] font-medium text-sm">{user?.name}</div>
                <div className="text-[#5f6368] text-xs mt-1">{user?.email}</div>
              </div>
              <button
